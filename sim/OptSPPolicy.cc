...
money_t OptSPPolicy::returnExpectedPayment(Function* budgetFunction, double duration)
{
...
money_t OptSPPolicy::computeMarginalReturn(std::list<Job*> &jobPtrList)
{
...
money_t OptSPPolicy::computeMarginalReturn(std::list<Job*> &jobPtrList, IloCplex& cplex, IloArray<IloNumVarArray>& x,
        IloInt nbTimeUnits)
{
    money_t marginalReturn = 0;
    IloInt j = 0;
...
money_t OptSPPolicy::computeTotalReturn(std::list<Job*> &jobPtrList)
{
...
bool OptSPPolicy::allocateTokensToJobs(std::list<Job*> &jobPtrList, token_t totalNumOfTokens)
{
...
//        IloInt M = 1000000; // a very large number
        ...
        for (j = 0; j < nbJob; j++)
        {
            ...
            IloNum Z = - atof(ConfigReader::getInstance().getVal("sp_quantile").c_str());
            IloNum mu = (*itr)->var_nor->distribution().mean();
            IloNum sigma = (*itr)->var_nor->distribution().sigma();
            IloNum para = mu + Z * sigma;
            IloNum error = Config::JOBSIZE_ERROR;
//            model.add(para * xaSum >= remainingJobSize - error);
            model.add(xaSum >= remainingJobSize / para);
...
void OptSPPolicy::reallocateTokensToJobs(std::list<Job*> &jobPtrList, token_t totalNumOfTokens)
{
    // do nothing
}
